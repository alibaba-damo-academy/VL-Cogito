...
def r1v_compute_score(predict_str: str, ground_truth: str, valid_response_ids = [], validation: bool = False, prompt_str: str = None) -> float:
    acc_reward = r1v_accuracy_reward(predict_str, ground_truth, prompt_str)
    if validation:
        return acc_reward
    format_reward = r1v_format_reward(predict_str)
    reward = acc_reward + format_reward * 0.5
    #return reward
    return {'all_reward':reward, 'acc_reward':acc_reward, 'format_reward':format_reward}


def r1v_acc_score(predict_str: str, ground_truth: str, valid_response_ids = [], validation: bool = False, prompt_str: str = None) -> float:
    acc_reward = r1v_accuracy_reward(predict_str, ground_truth, prompt_str)
    return {'all_reward':acc_reward, 'acc_reward':acc_reward}
