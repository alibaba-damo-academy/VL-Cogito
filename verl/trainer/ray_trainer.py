...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, weights=None, gamma=1.0, lam=1.0):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=token_level_rewards, eos_mask=response_mask, index=index, weights=weights
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
        ...
        response_mask = attention_mask[:, -response_length:]
        advantages, returns = core_algos.compute_grpo_outcome_advantage(
            token_level_rewards=token_level_rewards, eos_mask=response_mask, index=index, weights=weights, version="Dr"
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
...
class RayPPOTrainer:
    ...
    def _validate(self) -> Dict[str, Any]:
        ...
        for test_data in self.val_dataloader:
            ...
            reward_tensor, inf_dict = self.val_reward_fn(test_batch)
    ...
    def fit(self):
        ...
        #add adv_estimator global env
        os.environ['adv_estimator'] = self.config.algorithm.adv_estimator
        os.environ['tar_len'] = str(self.config.worker.reward.tar_len)
        print('using adv estimator: ', os.environ['adv_estimator'])
        print('using tar_len: ', os.environ['tar_len'])
        ...
        for _ in range(self.config.trainer.total_episodes):
            for batch_dict in self.train_dataloader:
                self.global_step += 1
                os.environ['global_step'] = str(self.global_step)
                ...
                print('batch init lenth: ', len(batch))
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        # we combine with rule-based rm and prompt weight
                        reward_tensor, batch_inf_dict = self.reward_fn(batch)
                        batch.batch["token_level_scores"] = reward_tensor


                        def r1v_length_weight(batch_inf_dict, false_coff = 0.25):
                            batch_len = len(batch_inf_dict['sample_len'])
                            prompt_weight = {}
                            for i in range(batch_len):
                                if batch_inf_dict['prompt_avg_acc'][i] == 0:
                                    prompt_weight[batch_inf_dict['uid'][i]]=false_coff
                                else:
                                    prompt_weight[batch_inf_dict['uid'][i]]=1.0

                            return prompt_weight

                        def r1v_cir_weight(batch_inf_dict, false_coff = 0.25):
                            batch_len = len(batch_inf_dict['sample_len'])
                            prompt_weight = {}
                            for i in range(batch_len):
                                if batch_inf_dict['prompt_avg_acc'][i] == 0:
                                    ...
                                    if batch_inf_dict['prompt_avg_acc'][i] > 0.5 and batch_inf_dict['prompt_avg_acc'][i] <= 1:
                                        cos_weight = math.cos((batch_inf_dict['prompt_avg_acc'][i]-0.5) * math.pi)
                                        prompt_weight[batch_inf_dict['uid'][i]]=cos_weight
                                    else:
                                        prompt_weight[batch_inf_dict['uid'][i]]=1.0
                            return prompt_weight    


                        def r1v_cir_weight_all(batch_inf_dict, cur_type, false_coff = 0.25):
                            batch_len = len(batch_inf_dict['sample_len'])
                            prompt_weight = {}
                            if cur_type == 's_1':
                                for i in range(batch_len):
                            ...
                            if cur_type == 's_2':
                                for i in range(batch_len):
                                    if batch_inf_dict['prompt_avg_acc'][i] == 0:
                                        prompt_weight[batch_inf_dict['uid'][i]]=0
                                    else:
                                        cos_weight = math.cos((batch_inf_dict['prompt_avg_acc'][i]-0.5) * math.pi)
                                        prompt_weight[batch_inf_dict['uid'][i]]=cos_weight
                                return prompt_weight   
                            ...
                            if cur_type == 'h_1':        
                                for i in range(batch_len):
                                    if batch_inf_dict['prompt_avg_acc'][i] == 0:
                                        ...
                                        if batch_inf_dict['prompt_avg_acc'][i] <= 0.5 and batch_inf_dict['prompt_avg_acc'][i] > 0:
                                            prompt_weight[batch_inf_dict['uid'][i]]=1.0
                                        else:
                                            prompt_weight[batch_inf_dict['uid'][i]]=0.0
                                return prompt_weight       
                            if cur_type == 'h_2':        
                                for i in range(batch_len):
                                    if batch_inf_dict['prompt_avg_acc'][i] == 0:
                                        ...
                                        if batch_inf_dict['prompt_avg_acc'][i] <= 0.75 and batch_inf_dict['prompt_avg_acc'][i] >= 0.25:
                                            prompt_weight[batch_inf_dict['uid'][i]]=1.0
                                        else:
                                            prompt_weight[batch_inf_dict['uid'][i]]=0.0
                                return prompt_weight  
                        ...
                        if self.reward_fn.compute_score_name == 'r1v+length' or self.reward_fn.compute_score_name == 'r1v+minlength':
                            prompt_weight = r1v_length_weight(batch_inf_dict, false_coff=self.config.worker.reward.all_false_coff)
                        elif self.reward_fn.compute_score_name == 'r1v+length+cir' or self.reward_fn.compute_score_name == 'r1v+minlength+cir':
                            prompt_weight = r1v_cir_weight_all(batch_inf_dict, self.config.worker.reward.cur_type, false_coff=self.config.worker.reward.all_false_coff)
                        elif self.reward_fn.compute_score_name == 'r1v+cir':
                            prompt_weight = r1v_cir_weight_all(batch_inf_dict, self.config.worker.reward.cur_type, false_coff=self.config.worker.reward.all_false_coff)
                        else:
                            prompt_weight = None
