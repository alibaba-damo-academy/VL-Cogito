...
def compute_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor, eos_mask: torch.Tensor, index: torch.Tensor, epsilon: float = 1e-6, weights = None, version: str = None
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    if not weights:
        ...
        advantages = copy.deepcopy(scores)
        for i in range(bsz):
            advantages[i] *= weights[index[i]]
...
def compute_policy_loss(
    ...
    clipped_ratio = torch.exp(torch.clamp(negative_approx_kl, np.log(1.0 - cliprange), np.log(1.0 + cliprange + cliphigher)))
